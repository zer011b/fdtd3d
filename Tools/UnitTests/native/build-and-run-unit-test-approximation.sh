...
function build
{
  ...
  if [[ "$TOOLCHAIN_FILE_PATH" != "" ]]; then
    file ./Source/UnitTests/unit-test-approximation
    sudo rm -rf ${ROOTFS}/fdtd3d
    sudo mkdir -p ${ROOTFS}/fdtd3d
    sudo cp ./Source/UnitTests/unit-test-approximation ${ROOTFS}/fdtd3d
    sudo chroot ${ROOTFS} /fdtd3d/unit-test-approximation
  else
