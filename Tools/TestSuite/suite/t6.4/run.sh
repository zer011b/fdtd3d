...
function launch ()
{
  ...
  tmp_test_file=$(mktemp /tmp/vacuum2D_TEz.XXXXXXXX.txt)
  cp ${SOURCE_DIR}/Examples/vacuum2D_TEz.txt $tmp_test_file
  ...
  if [ "$is_ok" != "1" ]; then
    echo "Test result is incorrect for Examples/vacuum2D_TEz.txt"
    ret=$((2))
  fi
  ...
  ./fdtd3d --cmd-from-file ${SOURCE_DIR}/Examples/vacuum2D_TEz_scattered.txt &> $output_file
  ...
  if [ "$is_ok" != "1" ]; then
    echo "Test result is incorrect for Examples/vacuum2D_TEz_scattered.txt"
    ret=$((2))
  fi
