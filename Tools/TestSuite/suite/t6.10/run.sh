...
function launch ()
{
  ...
  tmp_test_file=$(mktemp /tmp/vacuum1D_EyHx.XXXXXXXX.txt)
  cp ${SOURCE_DIR}/Examples/vacuum1D_EyHx.txt $tmp_test_file
  ...
  ./fdtd3d --cmd-from-file ${SOURCE_DIR}/Examples/vacuum1D_EyHx_scattered.txt &> $output_file
  ...
  if [ "$is_ok" != "1" ]; then
    echo "Test result is incorrect for Examples/vacuum1D_EyHx_scattered.txt"
    ret=$((2))
  fi
