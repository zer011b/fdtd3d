...
if [[ "$USED_MODE" -eq "1" ]]; then
  MODE="-DSOLVER_DIM_MODES=DIM3 -DCUDA_ENABLED=ON -DCUDA_ARCH_SM_TYPE=sm_35"
elif [[ "$USED_MODE" -eq "2" ]]; then
  MODE="-DPARALLEL_GRID=ON -DPARALLEL_BUFFER_DIMENSION=x -DLINK_NUMA=ON -DPARALLEL_GRID_DIMENSION=3"
fi
