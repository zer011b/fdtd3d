...
BUILD_SCRIPT="cmake $SOURCE_DIR $CUDA_MODE -DPARALLEL_GRID_DIMENSION=3 -DCMAKE_BUILD_TYPE=Release -DVALUE_TYPE=d -DCOMPLEX_FIELD_VALUES=OFF -DPRINT_MESSAGE=ON -DCXX11_ENABLED=ON; make fdtd3d"
$BASE_DIR/build-base.sh "$TEST_DIR" "$BUILD_DIR" "$BUILD_SCRIPT"
if [ $? -ne 0 ]; then
  exit 1
fi
