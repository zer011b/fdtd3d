...
function launch ()
{
  ...
  ./fdtd3d --time-steps $timesteps --sizex $size --same-size --2d-tey --angle-phi 0 --dx $dx --wavelength $lambda \
  ...
  local max_diff=$(cat /tmp/$size.$dx.txt | grep "DIFF NORM Ez" | awk '{if (max < $14) {max = $14}} END{printf "%.20f", max}')
