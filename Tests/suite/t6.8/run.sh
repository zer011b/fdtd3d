...
function launch ()
{
  ...
  tmp_test_file=$($RUNNER_NATIVE mktemp /tmp/vacuum1D_ExHy.XXXXXXXX.txt)
  cp ${SOURCE_DIR}/Examples/vacuum1D_ExHy.txt tmp.txt
  echo $MODE >> tmp.txt
  ...
  val_max=$($RUNNER_RES cat "$RESPATH"previous-1_[timestep=100]_[pid=0]_[name=Ex]_[mod].txt | awk '{print $1}')
  val_min=$($RUNNER_RES cat "$RESPATH"previous-1_[timestep=100]_[pid=0]_[name=Ex]_[mod].txt | awk '{print $2}')
  ...
  if [ "$is_ok" != "1" ]; then
    echo "Test result is incorrect for Examples/vacuum1D_ExHy.txt"
    ret=$((2))
  fi
  ...
  cp ${SOURCE_DIR}/Examples/vacuum1D_ExHy_scattered.txt tmp.txt
  echo $MODE >> tmp.txt
  echo "--layout-type $layout_type" >> tmp.txt
  $RUNNER_RES cp tmp.txt "$ROOTFS"$tmp_test_file
  ...
  if [ "$is_ok" != "1" ]; then
    echo "Test result is incorrect for Examples/vacuum1D_ExHy_scattered.txt"
    ret=$((2))
  fi
