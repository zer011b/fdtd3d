...
function launch ()
{
  ...
  tmp_test_file=$($RUNNER_NATIVE mktemp /tmp/vacuum2D_TMz.XXXXXXXX.txt)
  cp ${SOURCE_DIR}/Examples/vacuum2D_TMz.txt tmp.txt
  echo $MODE >> tmp.txt
  ...
  if [ "$is_ok" != "1" ]; then
    echo "Test result is incorrect for Examples/vacuum2D_TMz.txt"
    ret=$((2))
  fi
  ...
  cp ${SOURCE_DIR}/Examples/vacuum2D_TMz_scattered.txt tmp.txt
  echo $MODE >> tmp.txt
  echo "--layout-type $layout_type" >> tmp.txt
  $RUNNER_RES cp tmp.txt "$ROOTFS"$tmp_test_file
  ...
  if [ "$is_ok" != "1" ]; then
    echo "Test result is incorrect for Examples/vacuum2D_TMz_scattered.txt"
    ret=$((2))
  fi
