...
function launch ()
{
  ...
  tmp_test_file=$(mktemp /tmp/vacuum3D.XXXXXXXX.txt)
  cp ${SOURCE_DIR}/Examples/vacuum3D.txt $tmp_test_file
  ...
  cp ${SOURCE_DIR}/Examples/vacuum3D_scattered.txt $tmp_test_file
  echo $MODE >> $tmp_test_file
  echo "--layout-type $layout_type" >> $tmp_test_file
