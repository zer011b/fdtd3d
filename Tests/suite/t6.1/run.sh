...
function launch ()
{
  ...
  tmp_test_file=$($RUNNER_NATIVE mktemp /tmp/vacuum3D.XXXXXXXX.txt)
  cp ${SOURCE_DIR}/Examples/vacuum3D.txt tmp.txt
  echo $MODE >> tmp.txt
  ...
  cp ${SOURCE_DIR}/Examples/vacuum3D_scattered.txt tmp.txt
  echo $MODE >> tmp.txt
  echo "--layout-type $layout_type" >> tmp.txt
  $RUNNER_RES cp tmp.txt "$ROOTFS"$tmp_test_file
