...
function launch ()
{
  ...
  $RUNNER "$RUNPATH"fdtd3d $MODE --time-steps $num_time_steps --size x:$sizez --1d-ezhy --angle-teta 90 --angle-phi 0 --angle-psi 90 \
    ...
    --norm-start x:$norm_start --norm-end x:$norm_end --calc-${exp_type}-ezhy-diff-norm --layout-type $layout_type &> $output_file
