...
function launch ()
{
  ...
  tmp_test_file=$($RUNNER_NATIVE mktemp /tmp/vacuum1D_EyHx.XXXXXXXX.txt)
  cp ${SOURCE_DIR}/Examples/vacuum1D_EyHx.txt tmp.txt
  echo $MODE >> tmp.txt
  ...
  cp ${SOURCE_DIR}/Examples/vacuum1D_EyHx_scattered.txt tmp.txt
  echo $MODE >> tmp.txt
  echo "--layout-type $layout_type" >> tmp.txt
  $RUNNER_RES cp tmp.txt "$ROOTFS"$tmp_test_file
